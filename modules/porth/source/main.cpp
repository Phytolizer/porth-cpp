...
void simulateProgram(const std::vector<porth::Op>& program, bool debugMode) {
    static_assert(porth::OpIds::Count.discriminant == 31, "Exhaustive handling of OpIds in simulateProgram");
...
int compileProgram(const std::vector<porth::Op>& program, const std::string& outFilePath) {
    ...
    static_assert(porth::OpIds::Count.discriminant == 31, "Exhaustive handling of OpIds in compileProgram");
...
porth::Op parseTokenAsOp(const porth::Token& token) {
    ...
    static_assert(porth::OpIds::Count.discriminant == 31, "Exhaustive handling of OpIds in parseTokenAsOp");
    if (word == "+") {
        return porth::plus();
    }
    if (word == "-") {
...
std::vector<porth::Op> crossReferenceBlocks(std::vector<porth::Op>&& program) {
    ...
    static_assert(porth::OpIds::Count.discriminant == 31, "Exhaustive handling of OpIds in crossReferenceBlocks");
    for (size_t ip = 0; ip < program.size(); ++ip) {
